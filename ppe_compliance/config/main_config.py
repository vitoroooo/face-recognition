...
def validate_configuration(config: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    if retention_min < 731 or retention_max > 735:
        validation_results["errors"].append(f"Data retention must be within 731-735 days tolerance")
        validation_results["valid"] = False
    ...
    if max_cameras < 8:
        validation_results["warnings"].append(f"System supports fewer than required 8 concurrent cameras")
